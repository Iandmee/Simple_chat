...
def login() -> json:
    ...
    if (
        login == None
        or password == None
        or len(login) > login_length
        or len(password) > password_length
        or not check_login_and_password(login, password)
    ):
...
def register() -> json:
    ...
    if login == None or password == None or password_check == None:
        return jsonify(
            status_message="Invalid Login/Password/Password_check!", status=0
        )
...
def create() -> json:
    ...
    if chat_name == None or password == None or password_check == None:
        return jsonify(
            status_message="Invalid Chat_name/Password/Password_check!", status=0
        )
    if password_check != password:
        return jsonify(status_message="Passwords don't match!", status=0)
...
def get_messages() -> json:
    ...
    try:
        ...
        return jsonify(
            ...
            status=1,
            messages=messages_and_last_message_id["messages"],
            last_message_id=messages_and_last_message_id["last_message_id"],
        )
    except Exception as e:
        print(e)
    ...
    for i in range(10):
        ...
        if messages_and_last_id != None and len(messages_and_last_id["messages"]) != 0:
            return jsonify(
                status_message="Success!",
                status=1,
                messages=messages_and_last_id["messages"],
                last_message_id=messages_and_last_id["last_message_id"],
            )
