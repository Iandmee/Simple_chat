...
def send(chat_id: str, login: str, message: str) -> bool:
    ...
    try:
        ...
        message_to_add = Message(
            chat_id=chat_id, message_text=message, user_login=login
        )
...
def get(chat_id: str, count: int = 10) -> Optional[dict]:
    ...
    try:
        messages = (
            Chat.query.filter_by(chat_id=chat_id)
            .first()
...
def check_messages(
    chat_id: str, last_message_id: int, current_user_login: str
) -> Optional[dict]:
    """
    ...
    try:
        messages = (
            Chat.query.filter_by(chat_id=chat_id)
            .first()
            .messages.filter(Message.id > last_message_id)
            .all()
        )
        last_messages = []
        for message in messages:
