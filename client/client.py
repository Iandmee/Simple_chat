...
def set_options(**kwargs)->bool:
    ...
    try:
        ...
        file_with_options.writelines(base64.b64encode(json.dumps(current_options).encode()).decode()+"\n")
...
def api(**kwargs)-> json:
    ...
    if 'login' in data.values():
        return Client.session.post(get_url()+'/login',data={'login': data['login'],'password':data['password']}).text
    if 'register' in data.values():
        return Client.session.post(get_url() + '/register',
                                   data={'login': data['login'], 'password': data['password'], 'password_check': data['password_check']}).text
    if 'connect' in data.values():
        return Client.session.post(get_url() + '/connect',
                                   data={'chat_id': data['chat_id'], 'password': data['password']}).text
    if 'create' in data.values():
        return Client.session.post(get_url() + '/create',
                                   data={'chat_name': data['chat_name'], 'password': data['password'],'password_check': data['password_check']}).text
    if 'send_message' in data.values():
        return Client.session.post(get_url() + '/send_message', data={'message': data['message']}).text
    if 'get_messages' in data.values():
        if 'count' in data.keys():
            return Client.session.post(get_url() + '/get_messages', data={'count': data['count']}).text
        else:
            return Client.session.post(get_url() + '/get_messages').text
    if 'poll' in data.values():
        return Client.session.post(get_url() + '/poll',
                                   data={'last_message_id':data['last_message_id']}).text
...
def print_logged_in_menu() -> None:
    ...
    :return: None
    '''
    print(colored("### ", "magenta") + colored("Us3r: ", "cyan") + Style.BRIGHT + colored(Client.login, "green") + Style.RESET_ALL + colored(" ###", "magenta"))
...
def print_settings_menu()->None:
    ...
    :return:
    '''
    print(colored("### ", "magenta") + colored("Options", "cyan") + colored(" ###", "magenta"))
    print(colored("1. ", "cyan") + "Server ip/domain: "+ colored(Server.domain,'green'))
...
def print_messages()->None:
    ...
    while getattr(poll, "do_run", True):
        try:
            messages = json.loads(api(last_message_id=Client.last_message_id, type='poll'))
            if len(messages['messages'])>0:
                print('\n', end='')
                print("\033[A                             \033[A") #for deleting previous string in console
                for user, message in messages['messages']:
                    print(colored(f'({user})', "blue") +Style.BRIGHT + colored('> ', 'cyan') + base64.b64decode(message.encode()).decode())
                Client.last_message_id = messages['last_message_id']
                print(Style.BRIGHT + colored(f'({Client.login})', "green") + colored('> ', 'cyan'), end='')
        except Exception as e:
            pass
        sleep(1)
...
def chat_session(chat_name:str)-> None:
    ...
    :return:
    '''
    print(colored("### ", "magenta") + Style.BRIGHT + colored(f"Ch2t: {chat_name} ", "cyan") +Style.RESET_ALL + colored(" ###", "magenta"))
    try:
        ...
        if messages_and_last_message_id['status']:
            ...
            print(Style.BRIGHT + colored(messages_and_last_message_id['status_message'], 'red'))
            Client.chat_id = None
            return None
    except Exception as e:
    ...
    while 1:
        try:
            ...
            if user_message == "#Exit".lower():
                ...
                poll.do_run = False
                break
            status = json.loads(api(message=base64.b64encode(user_message.encode()).decode(), type='send_message'))
            if status['status']:
                print(colored(f'({Client.login})', "blue") + Style.BRIGHT + colored('> ', 'cyan') + user_message)
            else:
    ...
    while 1:
        try:
            ...
            if user_input == '1':
                ...
                status = json.loads(api(chat_id=chat_id, password=password,type='connect'))
                if status['status']:
                ...
                status = json.loads(api(chat_name=chat_name, password=password, password_check=password_check, type='create'))
                if status['status']:
                    print(Style.BRIGHT + colored(status['status_message'], 'green'))
                    print(Style.BRIGHT + colored("(REMEMBER IT!)","red")+colored(f"Your chat_id: {status['chat_id']}"))
                else:
                    print(Style.BRIGHT + colored(status['status_message'], 'red'))
            elif user_input == '3':
...
def recover_session(options:dict)->bool:
    ...
    if 'login' in options.keys() and 'password' in options.keys():
        ...
        if user_input == 'Y':
            try:
                status = json.loads(api(login=Client.login, password=Client.password, type='login'))
                if status['status']:
                    print(Style.BRIGHT + colored(status['status_message'], 'green'))
                else:
                    print(Style.BRIGHT + colored(status['status_message'], 'red'))
...
def save_creds_question()->None:
    ...
    :return:
    '''
    print(Style.BRIGHT + colored("(Unsafe!)", "red") + "Save your login and password?(Y/n): ", end='')
    user_input = sys.stdin.readline().strip()
    if user_input == 'Y':
    ...
    while 1:
        try:
            ...
            if user_input == '1':
                ...
                if set_options(domain=Server.domain,port=Server.port,secure=Server.secure):
                    print(Style.BRIGHT + colored("Saved!", 'green'))
                else:
    ...
    while 1:
        try:
            ...
            if user_input == '1':
                ...
                password_check = sys.stdin.readline().strip()
                status = json.loads(api(login=Client.login, password=Client.password, password_check=password_check, type='register'))
                if status['status']:
                    print(Style.BRIGHT + colored(status['status_message'], 'green'))
                    save_creds_question()
                    logged_in()
